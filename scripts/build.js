...
const SIGNING_KEY_PATH = path.join(os.homedir(), '.mellooessentials-signing', 'private_key.pem');

const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
const ask = (question) => new Promise((resolve) => rl.question(question, resolve));

(async () => {
  console.log('MellooEssentials build helper\n');
  ...
  if (!wantsDev) {
    ...
    console.log('\nQuestions about MellooEssentials? Reach hexedmaya:');
    console.log('  Discord: hexedmaya');
    console.log('  Email:   maja@melloo.me');
    console.log('  GitHub:  https://github.com/SkyMelloo/MellooEssentials\n');
  } else {
    ...
    const tmpFile = path.join(os.tmpdir(), `mellooessentials-changelog-${Date.now()}.txt`);
