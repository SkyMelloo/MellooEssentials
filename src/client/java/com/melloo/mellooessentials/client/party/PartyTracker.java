...
public final class PartyTracker {
	private static final Logger LOGGER = LoggerFactory.getLogger("MellooEssentials/PartyTracker");
	...
	public static void init() {
		...
		HypixelModAPI.getInstance().createHandler(ClientboundPartyInfoPacket.class, packet -> {
			...
			LOGGER.debug("Party info request failed (" + error + ") - keeping last known party state.");
		});
		...
		ClientReceiveMessageEvents.GAME.register((message, overlay) -> {
			...
			if (CONFIRMED_LEFT_PARTY.matcher(text).find()) {
				...
				leaderUuid = null;
				LOGGER.debug("Confirmed party leave/disband via chat - clearing cached party state immediately.");
			}
