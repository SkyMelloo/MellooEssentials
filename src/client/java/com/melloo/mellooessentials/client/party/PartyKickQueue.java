...
public final class PartyKickQueue {
	...
	private static final int KICK_INTERVAL_TICKS = 22; // ~1.1s, a small margin over Hypixel's own ~1s cooldown
	private static final Pattern KICK_COOLDOWN_FAILED = Pattern.compile("Command Failed:.*cooldown", Pattern.CASE_INSENSITIVE);

	// Optional extra text/buttons another mod wants appended to the join-notification line - null means nothing extra.
	...
	public static void init() {
		...
		ClientReceiveMessageEvents.GAME.register((message, overlay) -> {
			...
			if (KICK_COOLDOWN_FAILED.matcher(message.getString()).find()) {
				// Requeue at the front so it's retried next, and wait a full fresh interval.
	...
	// Deduped against both the pending queue and whatever kick is currently awaiting confirmation.
	...
	// The caller decides WHEN a join counts as new; this decides WHAT to do about it. A blocked user
	// is auto-kicked here instead of ever showing the buttons.
