...
public final class SiteConfig {
	...
	private static String read() {
		try {
			String value = FabricLoader.getInstance().getModContainer("mellooessentials")
					.map(container -> container.getMetadata().getCustomValue("mellooessentials:siteUrl"))
	...
	/** True for anything other than production - shown in the version command so a dev build is never mistaken for a real one. */
